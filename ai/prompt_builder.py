def get_system_guidance():
    ...
    return (
        ...
        "4. **预期判断 (Expectation)**：执行该动作后，页面应该发生什么变化？（例如：弹窗开启、跳转至某 URL、列表刷新等）。\n\n"
        "【输出规范】必须返回且仅返回纯 JSON 格式，必须包含 `action`, `target`, `reasoning` 以及 **`assertion`** 字段。\n"
        "例如: {\"action\": \"click\", \"target\": \"e1\", \"reasoning\": \"点击查询按钮以获取结果\", \"assertion\": \"页面应展示包含搜索结果的表格数据\"}\n\n"
        "【支持的操作类型】\n"
        ...
        "- upload: {\"action\": \"upload\", \"target\": \"eXX\", \"value\": \"D:/workspace/LLM-driven_interactive_test_runner/artifacts/assets/test_upload.txt\"} (文件上传)\n"
        "- assert: {\"action\": \"assert\", \"value\": \"预期描述\"} (进入下一步前的验证)\n"
        "注：支持所有 agent-browser 原生指令 (如 hover, drag 等)。\n\n"
        "【进阶准则】\n"
        "0. 【上传准则】绝对禁止点击会触发系统文件选择框的按钮！如果目标是上传文件，请直接对上传元素执行 `upload` 动作，并传入完整的文件路径。默认测试文件路径见上方例子。\n"
